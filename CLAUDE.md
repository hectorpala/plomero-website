...
- **Quién DECIDE el negocio**: el panel `subagent_type: decisor-negocio` (`.claude/agents/decisor-negocio.md`) = dev senior + maestro plomero/experto de Culiacán. Lee NEGOCIO.md + demanda GSC y decide crear/enriquecer/escalar (FASE 6). Decide; el backbone determinista ejecuta y los candados verifican.
...
- CSS: hoja ÚNICA `styles.min.css` (consolidada 19-ago-2026; styles.css y styles.7f293647.css eliminadas, con 301 en _redirects). Al cambiar estilos: editar SOLO styles.min.css, versionar URL (?v=AAAAMMDD) en TODAS las páginas Y subir versión del service worker (sw.js).
...
- AUTONOMÍA AMPLIADA (2026-06-21): `auto-fixers.py` ahora tiene ASSET FIXERS de CSS compartido (p.ej. `tap-target-44`) que editan el CSS único y **bumpean `?v=` en las ~70 páginas + `sw.js`** (el CSS es `immutable` 1 año → cambiar contenido NO basta, hay que cambiar la URL). `.pipeline/limpiar-huerfanos.py run --apply` borra artefactos huérfanos (`*.min.html`/`*.bak`) solo si pasan 4 condiciones (cero refs + no en sitemap + git-trackeado). El agente ya NO punto-humano estos dos. RED de seguridad en `.git/hooks/pre-push` (copia rastreada en `.pipeline/hooks/`, instala con `install.sh`): ABORTA si se borra una página viva (referenciada/en sitemap) o la home. El cap de 18 de FASE 8 cuenta PÁGINAS HTML; un cambio de asset (CSS+?v=+sw.js) o un borrado de huérfano NO cuentan.
