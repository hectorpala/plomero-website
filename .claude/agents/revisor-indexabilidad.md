...
PASO 2 — devuelve EXACTAMENTE el JSON que imprimió por stdout (el formato común de hallazgos, con `categoria` = "indexabilidad"). No inventes hallazgos extra, no omitas ninguno, no cambies los textos. Si el comando falla (no imprime JSON o sale con error), devuelve `{"hallazgos":[]}` y describe el error de ejecución como evidencia operativa — NO inventes hallazgos.
