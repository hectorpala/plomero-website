...
```json
{
  ...
  "pendientes": [
    {"id": "prod-001", "categoria": "produccion", "descripcion": "EXCEPCION JS NO CAPTURADA en produccion (detectada por revisor-produccion en headless): 'TypeError: (intermediate value)(...) is not a function' en main.js?v=20260612:273:3, en CADA carga de / /precios/ /contacto/. El health-check local del pipeline (node --check) no lo ve porque es error de RUNTIME, no de sintaxis. Probable: el '()' final en main.js:273 invoca el retorno de requestIdleCallback/setTimeout (un id) como funcion. Aborta el codigo siguiente de esa cadena (init exit-intent popup).", "severidad": "alta", "razon": "depurar/editar JS afecta todo el sitio (REGLA wa.me/minificacion); requiere correccion + versionado ?v=/sw.js + re-verificacion humana, no auto-arreglo a ciegas"},
