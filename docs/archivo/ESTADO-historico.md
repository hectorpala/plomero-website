# ESTADO — histórico archivado

Corridas anteriores al 2026-08-28, sacadas de `docs/ESTADO.md` el 2026-09-04.
Motivo: ESTADO.md se inyecta como preámbulo en CADA una de las ~1,100 llamadas de una corrida.
Con 31,532 tokens (5× su presupuesto) costaba ~28M de cache_read por corrida solo por el exceso.
Nada se pierde: el relato de cada incidente vive además en `data/HISTORIAL.jsonl`.
...
```json
{
  ...
  "modo": "AUTONOMO (diario completo: FASE 0-10), con un desvío grande respecto al flujo normal: FASE 0 encontró la rama auto/diario-20260723-0903 con 85 archivos STAGEADOS de una corrida del 2026-07-23 interrumpida ANTES de FASE 7-10 (razón probable: la sesión de Claude de ese día agotó su cuota). Dos corridas de RESPALDO (Codex, 2026-07-23 tarde y 2026-07-24) intentaron avanzarla pero se bloquearon por problemas de SU PROPIO sandbox (git checkout -b con 'cannot lock ref'/Operation not permitted; Chrome headless sin poder lanzar; kill -TERM sin permiso sobre el listener del puerto 8080) y solo dejaron documentado el bloqueo, sin tocar el sitio. Hoy: confirmado explícitamente que NINGUNO de esos 3 bloqueos existe en este entorno (rama de prueba creada y borrada sin error, check-e2e/check-produccion/check-tracking corrieron con Chrome headless real vía /Applications/Google Chrome.app, puerto 8080 libre) -> se decidió RETOMAR la rama en vez de descartarla o empezar de cero (su contenido ya había pasado por FASE 3-6 el 07-23/24, confirmado por .pipeline/oportunidades-20260723.md y -20260724.md).",
  "resumen": "Sin health check nuevo de FASE 1 clásico (el árbol ya tenía 85 archivos staged); en su lugar se corrió ci-gate (0 ALTA) y los 3 checkers dependientes de Chrome headless (check-e2e, check-produccion, check-tracking) para descartar que los bloqueos de Codex fueran reales aquí -- limpios, con las 5 alertas ALTA de check-tracking confirmadas como falso-positivo preexistente en producción (GTM no inicializa antes de la primera interacción por Consent Mode, reproducido igual contra un snapshot de main). FASE 7 (verificador SOLO-LECTURA, 1ra pasada): ok=false -- encontró UNA regresión real introducida por el trabajo del 07-23/24: al convertir `<h4>`→`<h3>` en `.footer-section` de precios/index.html y servicios/reparacion-de-fugas/index.html (intento de cerrar el pendiente humano a11y-301 'footer h4→h3', vigente desde antes del 2026-06-26), el texto heredaba el color oscuro de la regla global `h1,h2,h3{color:var(--text)}` en vez del color claro del footer (`.footer{color:#F8FAFC}`) -- contraste ~1:1, ilegible (confirmado con Chrome headless: rgb(15,23,42) vs el rgb(248,250,252) correcto de las páginas hermanas que seguían en `<h4>`). FASE 5 remediación: agregada la regla `.footer-section h3{color:#F8FAFC}` en las 3 hojas CSS compartidas + re-sincronizado el cache-busting con `python3 .pipeline/auto-fixers.py run --apply` (auto-reparo: el hash de css-bump-state.json había quedado desfasado tras el edit de CSS -> `?v=20260725` en 71 páginas + sw.js CACHE_NAME+1). 2da pasada del verificador: ok=true, 0 problemas. FASE 6: GSC revisado en vivo (579 clics +45%, 57301 impresiones +67%, posición 6.3); `gestor-backlog.py stats` con 0 auto-ejecutables; `check-plantilla.py` con 0 hallazgos totales (confirma que el backlog FAQPage-vs-visible de 12 páginas, pendiente desde 2026-07-21, quedó en 0 -- se resolvió como parte del trabajo de las corridas 07-23/24, incluyendo 4 contradicciones de precio corregidas contra el canónico precios/index.html); todas las oportunidades de gsc_opportunities mapean a páginas existentes -> 0 páginas nuevas.",
  "arreglados": "1 clase nueva (regresión de contraste footer h4→h3, 2 páginas + 3 CSS compartidos) sobre un fondo de 85 archivos ya trabajados el 07-23/24 (FAQPage-vs-visible en ~12 páginas + 4 contradicciones de precio + regresión de color de nav/breadcrumb + ancla + tap-target, ya documentados en HISTORIAL.jsonl de esas fechas). Detalle de hoy en HISTORIAL.jsonl (2 líneas fecha 2026-07-25).",
  "crecimiento": "0 páginas nuevas (GSC revisado en vivo vía MCP: toda la demanda real mapea a páginas existentes, mismo panorama que las corridas previas). 0 optimizaciones nuevas en el backlog persistente (gestor-backlog.py: 0 auto-ejecutables) -- el foco real de hoy fue destrabar y verificar 2 días de trabajo represado, no descubrir oportunidades nuevas. Ver .pipeline/oportunidades-20260725.md.",
  "verificado_ok": "true en la 2da pasada (verificador SOLO-LECTURA independiente, 2 pasadas): 1ra ok=false por la regresión de contraste del footer (única); remediada (regla CSS + re-bump) y re-verificada con Chrome headless real (rgb(248,250,252) en ambas páginas, página hermana no tocada intacta). 2da pasada ok=true 0 problemas: git diff main = 85 archivos exactos (0 páginas nuevas/borradas), ci-gate 0 ALTA, check-css-paridad OK (441 átomos/3 hojas), gate-pagina CANDADO OK en precios y reparacion-de-fugas (y 13 páginas más de muestra en la 1ra pasada), css-bump-state.json con hash recalculado a mano y coincidente, ?v=20260725 sin restos de tokens viejos en las 70 páginas, sw.js CACHE_NAME avanzado, canonical==og:url==twitter:url, 0 enlaces internos rotos, 0 electricista/GTM ajeno, email intacto, sitemap solo con lastmod actualizado.",
  "publicado": "SI, vía scripts/crecer.py publicar (secuencia segura: fetch + merge --ff-only + merge --no-ff + push, sin --force). 87 archivos en 1 commit (85 del trabajo 07-23/24 + el fix de contraste de hoy + 2 líneas de HISTORIAL.jsonl + REGLAS.md + este archivo pendiente de cerrar). El cap de 18 páginas de FASE 8 NO se disparó (la mayoría de las ~70 páginas HTML del diff solo cambiaron `?v=`, exentas por diseño de `_cap_paginas`; las páginas con cambio sustantivo real del 07-23/24 quedaron por debajo del cap). Pre-push encoló 68 URLs en GSC (0 en cola, 0 descartadas). Confirmado en producción real tras el deploy: precios/index.html sirve `?v=20260725`, HTTP 200 en precios/servicios/reparacion-de-fugas/home.",
  "pendientes_nuevos": "(auto, prioridad baja) a11y-301-footer-h4-h3-16-paginas-restantes: el pendiente humano a11y-301 (footer `<h4>`→`<h3>`) solo se completó en 2 de ~18 páginas conocidas (precios, reparacion-de-fugas) -- las ~16 restantes siguen en `<h4>` (funcionalmente correctas, solo inconsistentes en jerarquía semántica); si se retoma, replicar TAMBIÉN la regla `.footer-section h3{color:#F8FAFC}` ya centralizada en los 3 CSS (el riesgo de la regresión de hoy ya no aplica, la regla queda lista para cualquier página que se convierta). (sigue vigente, sin cambio hoy) seo-imagen-incorrecta-tecnico-gas, contenido-testimonios-posiblemente-inventados-boiler-gas, a11y-aria-label-menu-sin-acento-~63-paginas, contenido-precio-inconsistente-mantenimiento-boiler-basico (verificar si quedó resuelto por el trabajo de precios del 07-23/24, no confirmado explícitamente hoy), seo-107/GPS de zona-oriente/zona-poniente, rama-huerfana-auto-diario-20260718-pendiente-revision (sigue sin revisión humana, más vieja aún), secretos-en-historial-git, check-tracking GTM-no-inicializa-en-produccion (falso-positivo conocido por Consent Mode, pero real en el sentido de que produce ALTA en el checker cada corrida -- considerar silenciarlo con una nota explícita en vez de re-investigarlo cada vez).",
  "_corrida_anterior": {
  "fecha": "2026-07-21",
